...
func init() {
	CmdRun.Flags().StringVarP(&excludeDir, "excludeDir", "", excludeDir, `eg: scapr run --excludeDir="tmp,vendor,.git,.idea"`)
	CmdRun.Flags().StringVarP(&includeExt, "includeExt", "", includeExt, `eg: scapr run --includeExt="go,tpl,tmpl,html,yaml,yml,toml,ini,json"`)
	if excludeDir == "" {
...
var CmdRun = &cobra.Command{
	Use:     "run",
	Short:   "scapr run [main.go path]",
	Long:    "scapr run [main.go path]",
	Example: "scapr run cmd/server",
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Printf("\033[35mscapr run %s.\033[0m\n", dir)
...
func watch(dir string, programArgs []string) {
	...
	// Loop watching file modification
	for {
		select {
		case <-quit:
			err = killProcess(cmd)
